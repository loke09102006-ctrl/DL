...
result_folder = r"E:\DL project\UAVid_Dehazing\results\cnn"
model_path = r"E:\DL project\UAVid_Dehazing\models\cnn_dehazer.pth"
...
def dehaze_large_image(image, tile_size=256):
    ...
    for y in range(0, original_height, tile_size):
        for x in range(0, original_width, tile_size):
            ...
            # Replicate edge pixels so small edge tiles can become 256x256
            ...
            # Put the restored tile back in its original position
            output_image[:, :, y:y + tile_height, x:x + tile_width] = restored_tile
...
with torch.no_grad():
    ...
    for file in sorted(os.listdir(hazy_folder)):
        ...
        # Dehaze while keeping the original resolution
        output = dehaze_large_image(hazy_image, tile_size=256)

        # Convert GT to tensor without resizing
